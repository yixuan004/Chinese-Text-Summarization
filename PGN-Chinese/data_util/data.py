...
# create a vocab, contains functions id <==> word
class Vocab(object):
...
# data astype tensorflow.example
    ...
    while True:
        ...
        for f in filelist:
            ...
            while True:
                ...
                # the 'q' is for longlong, '%d' is for double, 's' is for char[]
...
# convert the words in article to ids
...
# convert the words in abstract to ids
def abstract2ids(abstract_words, vocab, article_oovs):
    ids = []
    unk_id = vocab.word2id(UNKNOWN_TOKEN)
    for word in abstract_words:
        i = vocab.word2id(word)
        if i == unk_id:  # If word is an OOV word
            if word in article_oovs:  # If word is an in-article OOV
                vocab_idx = vocab.size() + article_oovs.index(
                    word)  # Map to its temporary article OOV number
                ids.append(vocab_idx)
            else:  # If word is an out-of-article OOV
...
# if decoder produce some sentences(<s>xxx</s><s>xxx</s>). return them all.
...
def show_art_oovs(article, vocab):
    ...
    words = article.split(' ')
    words = [("__%s__" % word) if vocab.word2id(word) == unk_token else word
             for word in words]
    out_str = ' '.join(words)
    return out_str
