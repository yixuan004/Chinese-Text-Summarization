...
class Train(object):
    ...
    def train_one_batch(self, batch, iter, curr_step):
    ...
    # ------------------------------------------------------------------------------------
        ...
        loss = (self.opt.mle_weight * mle_loss + self.opt.rl_weight * rl_loss) / config.accumulation_steps
        loss.backward()
        if ((curr_step) % config.accumulation_steps) == 0:
            self.trainer.step()
    ...
    def trainIters(self):
        ...
        curr_count = count = mle_total = r_total = 0
        min_loss = 128
        min_count = 0
        while iter <= config.max_iterations:
            ...
            try:
                mle_loss, r = self.train_one_batch(batch, iter, curr_count)
            ...
            if iter % 50 == 0:
                ...
                logger.info("iter:" + str(iter) + "  mle_loss:" + "%.3f" % mle_avg + "  reward:" + "%.4f" % r_avg + min_flag)

                count = mle_total = r_total = 0

            if iter % 5000 == 0:
                self.save_model(iter)
            if curr_count - min_count >= config.require_improvement:
                logger.info("has not some improvement, early stop!")
                break
